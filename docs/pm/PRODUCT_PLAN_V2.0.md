...
2. **D7/D8 已决**：D7=`A`，合法 5 档卡 `CARD_PRESENTED` 即视为 offered；D8=`A+`，固定软去重 5、`5→4→3→2→1→0` 逐步放宽、仅 Host 显式选择洗牌、永不回退 V1.6 Router。两项均无残留待决状态。
...
6. **Human Gate 已决**：D1=V1.3 Frozen 350+40 成为 V2 内容真源；D2=relationship-aware 主线切换 V2 Router，旧 future deck/指数 selector 退役且生产不可达；D5=同一玩家每局最多 2 个 active、非排他 MATCH；D6=neutral/expansion 的 completed 轮只推进 `sessionCompletedRounds`，不推进 Heat/Signal/mutual interval。四项与 D3/D4/D7/D8 一并进入 `DEV_BASELINE=PRODUCT_PLAN_V2.0`。
...
5. V1.3 Frozen 明确扩圈 40/40 的 `fallbackPolicy=switch-to-table-version`。本终版已把规范性 T037 统一为 `switch-to-table-version`，并冻结 40/40 table-only 映射；v6 历史文档仅作审计记录，不再是 V2 执行口径。
...
Human Gate 已在 Phase1 P0=0、blocking P1=0 的前提下批准四项剩余决策并建立 `DEV_BASELINE=PRODUCT_PLAN_V2.0`。`PLAN_READINESS_SCORE=83` 未伪装为 90；Human 对 90 分阈值作一次性有条件例外，剩余 7 分对应的真机、真人与体验证据转入本文 Release 前强制 Gate。所有代码级证据仍是 DEVELOP 必过门禁。
...
6. 只公布 A↔B 的共同结果；单向结果计算后立即清除。同一玩家可有多个非排他 MATCH，但同一 Session 的 active MATCH 硬上限为 2；达到上限后不再为该玩家创建新 MATCH。
7. 合法 MATCH 且全局开放度=5 时，启动该 pair 的 5 档保障窗口；每张具体动作仍执行 current consent。
8. 切到 neutral pack 时关系推进暂停、状态不丢；切回继续。扩圈按统一 table-only fallback 执行，不进入 Heat/Pair/MATCH。
...
1. V1.3 Frozen `10-卡片元数据.json`（350）与 `10b-扩圈元数据.json`（40）是 V2 内容与运行参数的唯一 SSOT；精确 archive member 路径与 hash 门禁见「Data / API → JSON SSOT 与 Hash Gate」。开发必须先生成并验证新受控快照，再通过事务迁移切换，禁止无 Gate 直接覆盖生产题库。
...
| `NEUTRAL_CARD_COMPLETED` | neutral pack 完成 | `+1` | `+0` | 不改 Relationship Coverage；只更新 neutral 历史 | 消耗 20/25 Session 轮次；不推进 Heat/regular mutual/Signal |
| `NEUTRAL_CARD_SKIPPED_OR_SWAPPED` | neutral pack 跳过或换题 | `+0` | `+0` | 只更新 neutral 自身展示/used 历史 | 不消耗 Session 轮次，不推进 Relationship Engine |
| `EXPANSION_CARD_COMPLETED` | expansion 原版或 table-only 版完成 | `+1` | `+0` | 不改 Pair/Coverage/Signal；记录 expansion used ID | 消耗 20/25 Session 轮次；不推进 Heat/regular mutual/Signal |
...
3. **V2.0 规范性 T037（取代 v6 历史冲突句）**：`V2-T037｜扩圈 40 卡接入 10b metadata；不进 Heat / Pair Score / MATCH；拒绝、不适合或 Host 跳过外邀时，按该 cardId 立即执行 table-only 映射，fallbackPolicy 固定为 switch-to-table-version。`这是 V2 唯一 T037 执行口径；v6 的“拒绝走 table fallback”仅作历史审计记录且明确作废。若要改为“直接跳过”，必须改内容基线并走 Change C，Builder 不得自行解释。
...
6. Phase 1 不改业务代码、不跑 Release；Human Gate 批准仅建立 `DEV_BASELINE=PRODUCT_PLAN_V2.0`，未收到明确“第二阶段，开发”前不得启动 Builder 或业务代码修改。
...
5. expansion/neutral 不推进 Heat/Signal/mutual interval 已由 Human 冻结为 V2.0 执行口径，但其现场可理解性尚未完成真人验证。
...
| 扩圈 fallback 冲突 | P0已关闭 | 拒绝后流程违反 Frozen 安全规则 | R2 40/40 映射＋V2.0 规范性 T037；旧冲突句作废 |
...
6. 常规 Gate 要求 Readiness ≥90；本版在关键 brownfield 事实已验证、P0=0、blocking P1=0 后，由 Human 对 `83/100` 作一次性有条件例外批准，剩余验证不得豁免，全部进入 Release 前强制 Gate。

### Phase 2 Product DoD（`DEV_BASELINE=PRODUCT_PLAN_V2.0`）
...
### Release 前强制 Gate｜83 分例外批准的剩余 7 分
...
> Human Gate 已完成，D1～D8 全部冻结为 `DEV_BASELINE=PRODUCT_PLAN_V2.0`；本节保留决策回执，不再含待决项。

1. **D1｜内容切换｜已决**：V1.3 Frozen 350+40 成为 V2 运行真源；按 P0-01 的自动等价映射 `NONE` 与六条 migration policy 替换生产旧 350，旧卡只保留 history-only 兼容。
2. **D2｜动态 Router｜已决**：relationship-aware 主线切换本地逐轮 V2 Router；固定 future deck、`16:8:4:2:1` 与旧 selector 退役，生产路径必须不可达，异常/耗尽也不得回退旧 Router。neutral pack 可保留自身非关系 deck 能力，但不得接管 relationship-aware 路由。
...
5. **D5｜秘密互选与 MATCH｜已决 `上限 2`**：允许单设备传手机作为受控例外；MATCH 非排他，但每名玩家在同一 Session 最多 2 个 active MATCH，必须配套隐私遮罩、小局公平、原子 cap 校验与不泄露的 no-action 收尾。
6. **D6｜Expansion / Neutral｜已决 `中性不推进关系`**：两者 completed 轮计入 `sessionCompletedRounds`，但不推进 Heat/Signal/mutual interval；扩圈拒绝统一执行逐卡 table-only fallback。
...
- 产品目标与用户需求（20）：`19/20`。D1～D8 已全部拍板；pair 包容范围已明确限定为 V2.0 默认男女 pair，未来更包容模型另走变更。
...
- 开发范围与 DoD（10）：`9/10`。分层 DoD 清晰，P1-01～04 已标记契约关闭；仍需在 DEVELOP 中转成最终可执行 TASKS/traceability 并取得实现证据。
- 未决问题（5）：`4/5`。Human 决策已清零，P0=0、计划契约层 blocking P1=0；扣 1 分来自 Release 前真人验证尚未完成，不代表存在产品口径待决。
- 合计：`83/100`
- Gate（进 Human Review 条件）：Readiness >= 90 AND P0 = 0 AND blocking P1 = 0 AND 关键事实已验证 AND 核心假设已合理验证
- Human 例外回执：`83/100` 有条件批准；Phase1 可解冲突已全部关闭，P0=`0`、blocking P1=`0`，未获的 7 分不回填、不虚增，整体转入 Release 前强制 Gate，必须 `RG-01`～`RG-07` 全部通过。

## Research Review Round（第几轮/Reviewer 结论摘要）

- Round 1～5：Research Reviewer 已完成 P0-01、PlanConsistency、R2/R3/R4/R5 多轮复审；最终计划契约层结论为 P0=0、blocking P1=0，代码级证据转 DEVELOP 门禁。
- Human Gate：D1/D2/D5/D6 与 83 分例外均已拍板；终版将剩余验证固定为 Release 前 `RG-01`～`RG-07`，不得在实现或发布时降级。
