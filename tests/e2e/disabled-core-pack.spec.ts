...
test("禁用核心玩法后首页卡保留禁用态、点击不绕过、游戏包可重新启用", async ({ page }) => {
  await page.goto("/");
  await expect(page.getByRole("link", { name: /AI 即兴任务/ })).toBeVisible();

  // 在“游戏包”里禁用这个内置玩法（内置玩法与自定义玩法一样有启用开关）
  await page.goto("/packs");
  const toggle = page.getByLabel("启用AI 即兴任务");
  ...
  // 刷新后禁用状态仍在（本地持久化）
  await page.reload();
  await expect(page.getByLabel("启用AI 即兴任务")).not.toBeChecked();

  // 首页：卡片保留位置，但已是禁用态且不再是开局入口
  await page.goto("/");
  const disabled = page.getByRole("button", { name: /AI 即兴任务/ });
  await expect(disabled).toBeVisible();
  await expect(disabled).toHaveText(/已在游戏包禁用/);
  await expect(page.getByRole("link", { name: /AI 即兴任务/ })).toHaveCount(0);
  ...
  await page.getByLabel("启用AI 即兴任务").click();
  await expect(page.getByLabel("启用AI 即兴任务")).toBeChecked();
  await page.goto("/");
  await expect(page.getByRole("link", { name: /AI 即兴任务/ })).toBeVisible();
  await expect(page.getByRole("button", { name: /AI 即兴任务/ })).toHaveCount(0);
});
