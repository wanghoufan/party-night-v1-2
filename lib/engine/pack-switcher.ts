...
/** 用户在“游戏包”里启用的玩法（内置玩法当前恒为启用，自定义玩法按 enabled 标记）。 */
export function enabledPackIds(customPacks: CustomGamePack[] = []): string[] {
  return Array.from(createGamePackRegistry(customPacks).keys());
}
