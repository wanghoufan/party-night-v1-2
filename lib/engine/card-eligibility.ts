/**
 * B8 / D2｜出卡前的硬过滤与「换一个」去重（与出卡路由解耦）。
 *
 * 这些函数只回答「某张卡此刻能不能出 / 是不是刚被换掉」，不做任何加权抽取。
 * V1.6 的加权抽取（`INTENSITY_WEIGHT` / `pickWeightedCard` / `selectCard`）已随 D2 退役，
 * 只剩在 `lib/engine/card-selector.ts` 里做历史保留、生产不可达；出卡一律经 V2 Router
 * （`lib/engine/v2-deal.ts` → `drawV2SessionCard`）。
 */

import { BOUNDARIES } from "@/lib/domain/constants";
import type { BoundaryProfile, GameCard } from "./types";
...
/** 当前 Session 下这张卡是否通过全部硬过滤（used / 启用玩法 / 强度上限 / 人数 / 雷区）。 */
