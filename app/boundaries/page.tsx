...
export default function BoundariesPage() {
  ...
  async function start() { if (!draft) return; play("start-whistle"); const config = { ...draft, boundaries }; const session = createSession(config); await Promise.all([sessionRepository.save(session), preferencesRepository.save({ recentPlayers: config.players, lastSessionConfig: config })]); router.push(`/generating?session=${session.id}`); }
