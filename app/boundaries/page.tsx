...
export default function BoundariesPage() {
  ...
  async function start() { if (!draft) return; const customPacks = await gamePackRepository.list(); const customPackIds = draft.mode === "mixed" ? customPacks.filter((pack) => pack.enabled).map((pack) => pack.definition.id) : []; const config = { ...draft, boundaries, enabledPackIds: [...draft.enabledPackIds, ...customPackIds] }; const session = createSession(config); await Promise.all([sessionRepository.save(session), preferencesRepository.save({ recentPlayers: config.players, lastSessionConfig: config })]); router.push(`/generating?session=${session.id}`); }
