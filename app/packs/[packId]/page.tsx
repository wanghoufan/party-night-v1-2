...
/** 自定义玩法同样是不可信输入：保存前统一去控制字符、钳制长度，渲染时仍按纯文本交给 React（T197）。 */
function sanitizeCustomPack(pack: CustomGamePack): CustomGamePack {
  return {
    ...pack,
    definition: { ...pack.definition, name: sanitizeUntrustedText(pack.definition.name, 40) },
    cards: pack.cards.map((card) => ({
      ...card,
      content: sanitizeUntrustedText(card.content, UNTRUSTED_TEXT_LIMITS.cardContent),
      ...(typeof card.instruction === "string" ? { instruction: sanitizeUntrustedText(card.instruction, UNTRUSTED_TEXT_LIMITS.cardInstruction) } : {}),
    })),
  };
}
...
export default function PackEditorPage() {
  ...
  return <NeonBackground><main className="screen pack-editor"><header className="screen-header"><Link href="/packs" aria-label="返回游戏包"><Icon name="back" /></Link><h1>{id === "new" ? "新建游戏包" : "编辑游戏包"}</h1><span /></header><section className="pack-meta"><label><span>图标</span><input className="pack-icon-input" value={pack.definition.icon} maxLength={2} onChange={(event) => setPack({ ...pack, definition: { ...pack.definition, icon: event.target.value } })} /></label><label><span>名称</span><input value={pack.definition.name} maxLength={40} onChange={(event) => setPack({ ...pack, definition: { ...pack.definition, name: sanitizeUntrustedText(event.target.value, 40) } })} /></label></section><section className="card-editor-list"><div className="pack-section-heading"><h2>题卡</h2><button type="button" onClick={addCard}>＋ 添加题卡</button></div>{pack.cards.map((card, index) => <article key={card.id}><header><strong>题卡 {index + 1}</strong><button type="button" aria-label="删除题卡" onClick={() => setPack({ ...pack, cards: pack.cards.filter((item) => item.id !== card.id) })}><Icon name="trash" /></button></header><textarea value={card.content} placeholder="输入题目内容" onChange={(event) => setPack({ ...pack, cards: pack.cards.map((item) => item.id === card.id ? { ...item, content: sanitizeUntrustedText(event.target.value, UNTRUSTED_TEXT_LIMITS.cardContent) } : item) })} /><div><label>强度 <select value={card.intensity} onChange={(event) => setPack({ ...pack, cards: pack.cards.map((item) => item.id === card.id ? { ...item, intensity: Number(event.target.value) as Intensity } : item) })}>{[1,2,3,4,5].map((value) => <option key={value} value={value}>{value}</option>)}</select></label><label>参与者 <select value={card.participantMode} onChange={(event) => setPack({ ...pack, cards: pack.cards.map((item) => item.id === card.id ? { ...item, participantMode: event.target.value as GameCard["participantMode"] } : item) })}><option value="all">所有人</option><option value="single">单人</option><option value="pair">双人</option><option value="none">无需指定</option></select></label></div></article>)}</section><Button className="sticky-cta" type="button" disabled={!pack.cards.length || pack.cards.some((card) => !card.content.trim())} onClick={() => void save()}>保存游戏包</Button></main></NeonBackground>;
}
